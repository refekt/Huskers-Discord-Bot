...
async def process_error(ctx, error):
    ...
    if isinstance(err, commands.CommandNotFound):
        ...
        return await ctx.send(f"Command `${ctx.command.qualified_name}` received a bad argument. Review `$help {ctx.command.qualified_name}` for more information.")
    ...
    # elif type(error) == discord.ext.commands.CommandInvokeError:
...
async def monitor_messages(message: discord.Message):
    ...
    async def auto_replies():
        ...
        if re.search(r"f{1,}u{1,}c{1,}k{1,}[\W](y{1,}o{1,}u{1,}|u{1,})[\W]b{1,}o{1,}t{1,}", message.content, re.IGNORECASE):
            await channel.send(
                embed=build_embed(
        ...
        if re.search(r"l{1,}o{1,}v{1,}e{1,}[\W](y{1,}o{1,}u{1,}|u{1,})[\W]b{1,}o{1,}t{1,}", message.content, re.IGNORECASE):
            await channel.send(
                embed=build_embed(
        ...
        if re.search(r"b{1,}a{1,}d{1,}[\W]b{1,}o{1,}t{1,}", message.content, re.IGNORECASE):
            await channel.send(
                embed=build_embed(
                    title="╰（‵□′）╯",
        ...
        if 459569717430976513 in [roleid.id for roleid in message.author.roles]:
