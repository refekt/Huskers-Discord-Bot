...
async def get_prediction(user, recruit):
    ...
    """
    sql_response = Process_MySQL(
        query=get_prediction_query, values=(user.id, recruit._247_profile), fetch="one"
    )
    return sql_response


async def insert_prediction(
    user, recruit, team_prediction, prediction_confidence, previous_prediction
):
    if previous_prediction is None:
        ...
        """
        Process_MySQL(
            query=insert_prediction_query,
            values=(
                user.name,
                user.id,
                recruit.name,
                recruit._247_profile,
                recruit.year,
                team_prediction,
...
def get_croot_predictions(recruit):
    ...
    """
    get_croot_preds_response = Process_MySQL(
        query=get_croot_preds_query, values=recruit._247_profile, fetch="all"
    )
    return get_croot_preds_response


async def initiate_fap(
    ctx: Union[SlashContext, ComponentContext], user, recruit, client: discord_client
):
    await ctx.send(
        "Initiating FAP! Your inputs will be delete, but may appear to remain on the screen. They will disappear on reloading of Discord (Ctrl + R).",
        hidden=True,
    )

    if (recruit.committed.lower() if recruit.committed is not None else None) in [
        "signed",
        "enrolled",
    ]:
    ...
    if recruit.year < NO_MORE_PREDS:
        return await ctx.send(
            f"You cannot make predictions on recruits from before the [{NO_MORE_PREDS}] class. [{recruit.name}] was in the [{recruit.year}] recruiting class.",
            hidden=True,
        )
    ...
    while team_prediction is None:
        try:
            prediction_response_msg: discord.Message = await client.wait_for(
                "message",
            ...
            await ctx.send(
                "Sorry, you ran out of time. You'll have to initiate the FAP process again by clicking the crystal ball emoji on the crootbot message or using the $predict command.",
                hidden=True,
            )
            ...
            if prediction_response in [t.lower() for t in valid_teams]:
                ...
                if recruit.committed_school == team_prediction:
                    ...
                    return
                await ctx.send(
                    f"You've selected [{team_prediction}] as your prediction, what is your confidence in that pick from 1 to 10?",
                    hidden=True,
                )

            else:
                await ctx.send(
                    "That isn't a valid team. Please try again or ask my creators to add that as a valid team.",
                    hidden=True,
                )

    while prediction_confidence is None:
        try:
            confidence_response_msg: discord.Message = await client.wait_for(
                "message",
                check=lambda message: message.author == user
                and message.channel == ctx.channel,
                timeout=30,
            ...
            try:
                ...
                if 1 <= confidence <= 10:
                    await ctx.send(
                        f"You've selected [{confidence}] as your confidence level.",
                        hidden=True,
                    )
                    ...
                    await ctx.send(
                        f"{confidence}] is not between 1 and 10. Try again.",
                        hidden=True,
                    )
    ...
    await ctx.send(
        f"Your prediction of [{recruit.name}] to {team_prediction}] has been logged!",
        hidden=True,
    )
...
async def individual_predictions(ctx, recruit):
    ...
    individual_preds = Process_MySQL(
        query=get_individual_preds_query, fetch="all", values=(recruit._247_profile,)
    )
    ...
    for i, p in enumerate(individual_preds):
        ...
        pred_field = [
            f"{pred_user}",
            f"{p['team']} ({p['confidence']}) - {pred_dt.month}/{pred_dt.day}/{pred_dt.year}",
        ]
        if p["correct"] == 1:
...
class FAP_Commands(commands.Cog):
    ...
    async def leaderboard(self, ctx, year=None):
        ...
        get_all_preds_query = (
            f"SELECT * FROM fap_predictions WHERE recruit_class = {year}"
        )
        ...
        for u in leaderboard["user_id"].unique():
            ...
            leaderboard.loc[leaderboard["user_id"] == u, "correct_pct"] = (
            ...
            faps_user.loc[
                faps_user["time_delta"].dt.total_seconds() <= 864000, "points"
            ] = (
                faps_user.loc[
                    faps_user["time_delta"].dt.total_seconds() <= 864000, "correct"
                ]
                * faps_user.loc[
                    faps_user["time_delta"].dt.total_seconds() <= 864000, "confidence"
                ]
                * (
                    faps_user.loc[
                        faps_user["time_delta"].dt.total_seconds() <= 864000,
                        "time_delta",
                    ].dt.total_seconds()
                    / 864000
                )
            ...
            faps_user.loc[faps_user["correct"] == -1.0, "points"] = (
                faps_user.loc[faps_user["correct"] == -1.0, "confidence"] * -1
            )

            leaderboard.loc[leaderboard["user_id"] == u, "points"] = faps_user[
                "points"
            ].sum()
        ...
        for u in leaderboard["user_id"]:
            ...
            correct_pct = leaderboard.loc[
                leaderboard["user_id"] == u, "correct_pct"
            ].values[0]
    ...
    async def stats(self, ctx, target_member: discord.Member = None, year: int = None):
        ...
        get_stats_query = (
            f"""SELECT * FROM fap_predictions WHERE user_id = {target_member.id}"""
        )
        faps = Process_MySQL(query=get_stats_query, fetch="all")
        if faps is None:
            await ctx.send(
                "You have made no predictions previously. You can do so by calling `$predict <Recruiting Class> <Recruit Name>`"
            )
            return
        ...
        current_pct = (
            faps_df.loc[faps_df["recruit_class"] == year, "correct"].mean() * 100
        )
        ...
        current_correct_count = len(
            faps_df[(faps_df["correct"] == 1) & (faps_df["recruit_class"] == year)]
        )
        overall_wrong_count = len(faps_df[faps_df["correct"] == 0].index)
        current_wrong_count = len(
            faps_df[(faps_df["correct"] == 0) & (faps_df["recruit_class"] == year)]
        )

        avg_days_overall_str = ""
        avg_days_current_str = ""
        if (
            faps_df.loc[faps_df["correct"] == 1, "decision_date"].notna()
            * faps_df.loc[faps_df["correct"] == 1, "prediction_date"].notna()
        ).sum() > 0:
            timedeltas_correct_overall = (
                faps_df.loc[faps_df["correct"] == 1, "decision_date"]
                - faps_df.loc[faps_df["correct"] == 1, "prediction_date"]
            ).values
            avg_days_overall = (
                float(sum(timedeltas_correct_overall) / len(timedeltas_correct_overall))
                / 86400000000000
            )
        ...
        if (
            faps_df.loc[
                (faps_df["correct"] == 1) & (faps_df["recruit_class"] == year),
                "decision_date",
            ].notna()
            * faps_df.loc[
                (faps_df["correct"] == 1) & (faps_df["recruit_class"] == year),
            ...
            avg_days_current = (
                float(sum(timedeltas_correct_current) / len(timedeltas_correct_current))
                / 86400000000000
            )
        ...
        if overall_correct_count + overall_wrong_count > 0:
            overall_ratio_str = (
                f"{overall_correct_count}/{overall_correct_count + overall_wrong_count}"
            )
        if current_correct_count + current_wrong_count > 0:
            current_ratio_str = (
                f"{current_correct_count}/{current_correct_count + current_wrong_count}"
            )
        ...
        if np.isnan(overall_pct):
            ...
            embed_msg.add_field(
                name="**Overall**",
                value=f"Predictions: {overall_count}\nPercent Correct: {overall_pct}% ({overall_ratio_str})"
                + avg_days_overall_str,
                inline=False,
            )
        else:
            embed_msg.add_field(
                name="**Overall**",
                value=f"Predictions: {overall_count}\nPercent Correct: {overall_pct:.0f}% ({overall_ratio_str})"
                + avg_days_overall_str,
                inline=False,
            )
        if np.isnan(current_pct):
            ...
            embed_msg.add_field(
                name=f"**{year} Class**",
                value=f"Predictions: {current_count}\nPercent Correct: {current_pct}% ({current_ratio_str})"
                + avg_days_current_str,
                inline=False,
            )
        else:
            embed_msg.add_field(
                name=f"**{year} Class**",
                value=f"Predictions: {current_count}\nPercent Correct: {current_pct:.0f}% ({current_ratio_str})"
                + avg_days_current_str,
                inline=False,
            )
    ...
    async def user(self, ctx, target_member: discord.Member = None, year: int = None):
        ...
        user_preds = Process_MySQL(
            query=get_user_preds_query,
            values=(
                target_member.id,
                year,
            ),
        ...
        if user_preds is None:
            if target_member.id == ctx.author.id:
                await ctx.send(
                    f"You do not have any predictions for the {year} class. Get on it already."
                )
            else:
                await ctx.send(
                    f"{target_member.display_name} doesn't have any predictions for the {year} class."
                )
            return
        ...
        for i, p in enumerate(user_preds):
            ...
            if isinstance(pred_date, str):
                pred_date = datetime.datetime.strptime(
                    p["prediction_date"], DATETIME_FORMAT
                )
        ...
        for i, embed in enumerate(embed_list):
            ...
            if correct_amount + incorrect_amount > 0:
                ...
                embed_description = (
                    f"""Total {year} Predictions: {len(user_preds)}\n\n"""
                )
