...
async def on_message(message):
    ...
    #Started working on a HUDL highlight puller on react. This section is to take the crootbot message, find if a hudl profile exists, and pull the video. 
    #Next would be to monitor reactions and post the video if someone reacted to the video camera emoji.
    #I was thwarted because HUDL has everything wrapped in javascript, so we would need to consider adding in selenium or PhantomJS to finish this.
    #I'm keeping this for now since there is a chance we can continue forward with it someday and I don't want to re-engineer this piece. 
